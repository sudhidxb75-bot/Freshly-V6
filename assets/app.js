(function(){
  ...
  const demo = {
    ...
    banners:[
      {BannerID:'BAN001',Title:'Today’s Fresh Offer',Subtitle:'Fresh Fish & Seafood, meat and essentials delivered through your nearby hub.',ImageURL:'assets/images/banner-hub-partner.png',ButtonText:'Shop Now',ButtonLink:'#shop',SortOrder:1,Status:'Active',DisplayMode:'ImageOnly'},
  ...
  function bannerImageUrl_(b){
    // Desktop/default banner image. Mobile-specific fields are intentionally not first here.
  ...
  function bannerTitle_(b){
    return bannerValue_(b, ['Title','BannerTitle','Banner Title','Heading','Headline','Label']) || 'Freshness Delivered';
  }
  ...
  const mobileTopShift = String(b.MobileTopShift || b.MobileBannerTopShift || '0px').trim() || '0px';
    ...
    if(image && (displayMode === 'imageonly' || hideTextOverlay || !wantsTextOverlay)){
      ...
      return `<article class="promo-slide backend-banner image-only" style="${cssVars}" data-banner-source="${esc(b._BannerSource || '')}">
        <a class="banner-image-link" href="${btnLink}" aria-label="${title}">
    ...
    return `<article class="promo-slide backend-banner ${image?'has-image':''}" style="${cssVars}${bg}"><div class="banner-copy"><span class="eyebrow">${label}</span><h2>${title}</h2><p>${sub}</p></div><a class="btn btn-primary banner-main-btn" href="${btnLink}">${btnText}</a></article>`;
  }
  ...
  function saveLocalOrder_(res, order, items){
    ...
    rows.unshift({
      ...
      WhatsAppStatus: res.WhatsAppStatus || 'Demo', Items: items || []
    });
  ...
  function renderCustomerOrders_(orders){
    ...
    box.innerHTML = `<table class="table"><thead><tr><th>Order Number</th><th>Date</th><th>Items</th><th>Total</th><th>Status</th><th>Payment</th><th></th></tr></thead><tbody>${orders.map(o=>{
  ...
  function renderTrackOrder_(o){
    const no = o.OrderNumber || o.OrderFreshlyID || o.OrderID || '';
    const items = o.Items || o.items || [];
    return `<div class="card"><h3>Order ${esc(no)}</h3><p>Status: <b>${esc(o.Status||o.OrderStatus||o.TrackingStatus||'')}</b></p><p>Payment: <b>${esc(o.PaymentStatus||'')}</b></p><p>Total: <b>${currency}${num(o.Total||0)}</b></p><p><b>Slot:</b> ${esc(o.DeliverySlot||'')}<br><b>Hub:</b> ${esc(o.HubName||o.HubID||'')}</p>${items.length ? `<hr><p><b>Items</b><br>${orderItemsText_(items)}</p>` : ''}</div>`;
  }
